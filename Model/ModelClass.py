...
class Model(Data):
    def __init__(self, raw_Data_Matrix=[[1,2, 3],[3,4, 5]], cluster_Count=3, latent_Count=5, epsilon=1):
        ...
        self.Lg, self.Dg = init_Cholesky(self.G, self.q)
    ...
    def nBetaS(self, g):    
        ...
        while type(g) != int or not (0 <= g < self.G):
            ...
            try:
                ...
                continue
        # useful parameters
        L, D, Lamb = self.Lg[g], self.Dg[g], self.Lamb
    ...
    # unconstrained
    def up_Row_unConstrained(self, g, r):    # r = Ind(row) and g index starts from 0
        """updates a row"""
        while type(g) != int or not 0 <= g < self. G:
    ...
    def up_Dg(self):
        for g in range(self.G):
            ...
            self.Dg[g] = diag(diag(D))  # a diagonal matrix
        return 0
    ...
    def up_Lamb(self):
        ...
        for g in range(self.G):
            ...
            invL = inv(self.Lg[g])
            R = invL @ self.Dg[g] @ invL.T  # simplify by computing inverse once 
            for i in range(self.sample_Size):
                z = self.Z[i, g]
                X = self.data_Matrix[i,:]
